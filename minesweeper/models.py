...
class MinesweeperGame(models.Model):
    ...
    def make_visible_adjacent_squares(self, x, y):
        ...
        for x_offset in range(-1, 2):
            ...
            for y_offset in range(-1, 2):
                offset_y = y + y_offset
                if self.is_inside_board(offset_x, offset_y) and not self.is_visible(offset_x, offset_y):
    ...
    def set_board_visibility(self, state):
        """ Sets visibility for all locations on the board to match the passed state
        """
    ...
    def user_move(self, x, y, move_type='clear'):
        ...
        elif move_type == 'flag' and self.is_inside_board(x, y) and not self.is_visible(x, y):
